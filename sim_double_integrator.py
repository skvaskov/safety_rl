...
# car dynamics
parser.add_argument(
    "-c", "--constraint", help="constraint radius", default=[0, 0, 2, 2], type=List
)
parser.add_argument(
    "-t", "--target", help="target radius", default=[0.8, 0.0, 0.1, 1e-2], type=List
)
...
outFolder = os.path.join(args.outFolder, 'double_integrator-DDQN', fn)
print(outFolder)
...
env.set_constraint(args.constraint)
print("Dynamic parameters:")
print("  CAR", end='\n    ')
print(
    "Constraint: {:.1f} ".format(env.car.constraint_x_y_w_h[0])
    +", {:.1f} ".format(env.car.constraint_x_y_w_h[1])
    +", {:.1f} ".format(env.car.constraint_x_y_w_h[2])
    +", {:.1f} ".format(env.car.constraint_x_y_w_h[3])
    + "; Target: {:.1f} ".format(env.car.target_x_y_w_h[0])
    +", {:.1f} ".format(env.car.target_x_y_w_h[1])
    +", {:.1f} ".format(env.car.target_x_y_w_h[2])
    +", {:.1f} ".format(env.car.target_x_y_w_h[3])
    + "; Max accel: {:.1f}".format(env.car.acceleration_limits[0])
    +", {:.1f}".format(env.car.acceleration_limits[1])
)
print("  ENV", end='\n    ')
print(
    "Constraint: {:.1f} ".format(env.constraint_x_y_w_h[0])
    +", {:.1f} ".format(env.constraint_x_y_w_h[1])
    +", {:.1f} ".format(env.constraint_x_y_w_h[2])
    +", {:.1f} ".format(env.constraint_x_y_w_h[3])
    + "; Target: {:.1f} ".format(env.target_x_y_w_h[0])
    +", {:.1f} ".format(env.target_x_y_w_h[1])
    +", {:.1f} ".format(env.target_x_y_w_h[2])
    +", {:.1f} ".format(env.target_x_y_w_h[3])
)
...
if args.plotFigure or args.storeFigure:
  ...
  # = Value
  ax = axes[0]
  v = env.get_value(agent.Q_network, nx=nx, ny=ny)
  im = ax.imshow(
      v.T, interpolation='none', extent=axStyle[0], origin="lower",
      cmap='seismic', vmin=vmin, vmax=vmax, zorder=-1
  )
